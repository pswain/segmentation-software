function addAllTrapsLabelSecondaryCenterOnly(cDictionary,cTimelapse,channels,param,radius)

% channels(1) = 1 if DIC image is 1, and (2) = 2 if fluoresecnece is in 2
% param ='center' if you only want to label the center of the cells, and
% 'whole' if you want to label the whole cell;
...
switch param
    case 'center'
        ...
        for i=1:length(cTimelapse.cTrapsLabelled)
            cDictionary.cTrap(traps_in_dictionary+1).image=cTimelapse.returnSingleTrapTimelapse(i,channels(1));
            cDictionary.cTrap(traps_in_dictionary+1).class=logical(zeros(size(cDictionary.cTrap(traps_in_dictionary+1).image)));
                        
            trapTimelapse=cTimelapse.returnSingleTrapTimelapse(i,channels(2));
            ...
            if i==1
                thresh=graythresh(trapTimelapse)*.8;
            end
            
            figure(1); hbw=gca; figure(2);htp=gca;figure(3);haccum=gca;colormap(jet);
            disp(['Trap ',int2str(i)])
            for j=1:size(trapTimelapse,3)
                j
                trapTimepointDIC=trapTimelapse_DIC(:,:,j);
                trapTimepoint=trapTimelapse(:,:,j);
                t_im=trapTimepoint;
                trapTimepoint=imfilter(trapTimepoint,fspecial('disk',2),'replicate');
                
                bw=zeros(size(trapTimepoint,1),size(trapTimepoint,2))>0;
                bw_cells=im2bw(trapTimepoint,thresh);
                
                if (sum(bw_cells(:)))
                    bw_cells=imdilate(bw_cells,se2);
                    t_im=stdfilt(t_im,true(5));
                    t_im(:,:,2)=trapTimepointDIC;
                    [accum, circen] = CircularHough_Grd(t_im, [radius(1) radius(2)],max(t_im(:))*.001,6,1);
                    ...
                    for circles=1:size(circen,1)
                        if bw_cells(circen(circles,2),circen(circles,1))
                            temp=zeros(size(bw));
                            temp(circen(circles,2)-2,circen(circles,1)+1)=1;
%                             if cirrad(circles)>500
%                                 temp=imdilate(temp,se3);
%                             else
                                temp=imdilate(temp,se2);
%                             end
                            bw=bw|temp;
                        end
                    end
...
%                     bw=imdilate(bw,se);
                    ...
                    tempy=trapTimepoint;
                    tempy(bw)=tempy(bw)*2;
                    trapTimepointDIC(bw)=2*trapTimepointDIC(bw);
                    imshow(accum,[],'Parent',hbw);
                    imshow(bw_cells,[0 1],'Parent',htp);
                    imshow(trapTimepointDIC,[],'Parent',haccum);pause(.001);
                    cDictionary.cTrap(traps_in_dictionary+1).class(:,:,j)=bw>0;
                    cDictionary.labelledSoFar(i,j)=1;
                end
            end
        ...
        for i=1:length(cTimelapse.cTrapsLabelled)
            ...
            for j=1:size(trapTimelapse,3)
                ...
                bw=im2bw(trapTimepoint,thresh);
                        ...
                        imshow(trapTimepoint,[0 1],'Parent',htp);pause(.01);
                cDictionary.cTrap(traps_in_dictionary+1).class(:,:,j)=bw>0;
                cDictionary.labelledSoFar(i,j)=1;
            end
            traps_in_dictionary=traps_in_dictionary+1;
...
img1=img(:,:,1);
img2=img(:,:,2);
img2=padarray(img2,[2 0],median(img(:)),'post');
img2=padarray(img2,[0 1],median(img(:)),'pre');
img2(1:2,:)=[];img2(:,end)=[];

    [grdx, grdy] = gradient(img1);
    [grdx2, grdy2] = gradient(img2);
    
grdmag = sqrt(grdx.^2 + grdy.^2);
grdmag2 = sqrt(grdx2.^2 + grdy2.^2);
...
grdmag2=grdmag2/max(grdmag2(:))*max(grdmag(:));
weight4accum = ...
    repmat( double(grdmag(grdmasklin)) , [lin2accum_size(2),1] ) .* ...
    mask_valid_aJaI(:);
weight4accum2 = ...
    repmat( double(grdmag2(grdmasklin)) , [lin2accum_size(2),1] ) .* ...
    mask_valid_aJaI(:);
...
% accum2 = accumarray( lin2accum , weight4accum2 );
% accum2 = [ accum2 ; zeros( numel(grdmag2) - numel(accum2) , 1 ) ];
% accum2 = reshape( accum2, size(grdmag2) );
