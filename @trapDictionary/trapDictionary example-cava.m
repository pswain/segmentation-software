...
%%
cTrapDisplay(cDictionary,cCellVision)
%%

cTimelapse.cTimepoint=cTimelapse.cTimepoint(300:5:400);

%%
cTrapDisplay(cTimelapse,cCellVision)
%%
newtp=length(cTimelapse.cTimepoint);
cDictionary.cTimepoint(end+1:end+newtp)=cTimelapse.cTimepoint;
%%
cTimelapse.cTimepoint=cTimelapse.cTimepoint(1:6:300);
...
randTimepoints=[1 (ceil(rand(1,numTimepoints)*length(cTimelapse.cTimepoint)))];
randTimepoints=unique(randTimepoints);
cTimelapseSmall=cTimelapse;
cTimelapseSmall.cTimepoint=cTimelapseSmall.cTimepoint(randTimepoints);
