                    ...
                    if dupcellnumber>0%a cell was found at a previous timepoint at the position of thiscentroid(duptrackingnumber,:)
                        switch varargout{1}                                        
                            case 'none'%no duplicates were found in the fillgap method
                                ...
                                ind = find(obj.Segmented(:,:,timepoint)==duptrackingnumber);
                                obj.Tracked(ind+((size(obj.Segmented,1)^2)*(reftimepoint))) = dupcellnumber; %this linear indexing is ugly but the only way.
                            ...
                            case 'this'%a duplicate was found but this cell is closer
                                ...
                                obj.TrackingData(timepoint).cells(duptrackingnumber).cellnumber=dupcellnumber;
                                ind = find(obj.Segmented(:,:,timepoint)==duptrackingnumber);
                                obj.Tracked(ind+((size(obj.Segmented,1)^2)*(reftimepoint))) = dupcellnumber;
                                assignedcellnos(duptrackingnumber)=dupcellnumber;
                                ...
                                obj.Tracked(ind+((size(obj.Segmented,1)^2)*(reftimepoint))) = highest;
                       ...
                       ind = find(obj.Segmented(:,:,timepoint)==i);
                       obj.Tracked(ind+((size(obj.Segmented,1)^2)*(reftimepoint))) = obj.TrackingData(timepoint).cells(i).cellnumber;
                        ...
                        if cellnumber>0%a cell was found at a previous timepoint at the position of thiscentroid(i,:)
                            switch varargout{1}                                        
                                case 'none'%no duplicates were found in the fillgap method
                                    ...
                                    ind = find(obj.Segmented(:,:,timepoint)==i);
                                    obj.Tracked(ind+((size(obj.Segmented,1)^2)*(reftimepoint))) = cellnumber;
