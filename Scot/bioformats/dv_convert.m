function dv_convert(output_filename)
%small function to take a *.dv file and convert save each of the images
...
for series = 1:size(im,1)%loop over image series
    ...
    for tp = 1:(size(im{1,1},1)/2)%loope over timepoints
        ...
        %write image
        imwrite(im{series,1}{2*tp,1},[output_dir '/series' int2str(series) '/' output_filename '_' tp2str(tp,6) '_' channel '.png'],'png')
