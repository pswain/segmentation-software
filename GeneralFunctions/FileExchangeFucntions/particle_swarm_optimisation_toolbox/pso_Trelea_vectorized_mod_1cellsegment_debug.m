% ELCO EDITS 
% lines 99 - 105  -   new variable 'functname_string' so that
%                     'functname' can be a function handle.
% line 354        -   commented out eval(plotfcn) so that didn't get plots
%                     (too lazy to change the plotfcn variable)
...
%added display elements to aid in debugging/parameter selection
...
function [OUT,varargout]=pso_Trelea_vectorized_mod_1cellsegment_debug(functname,D,show_image,center,angles,varargin)

if isa(functname,'function_handle')
    functname_string = func2str(functname);
elseif isa(functname,'char')
    functname_string = functname;
else
...
% start PSO iterative procedures
 ...
 %added by elco for visualisation
 
 show_numbers = 5;%number of plots to show
 colours = ['r', 'g', 'b', 'c', 'm', 'y', 'k'];%colours of plots
 debug_figure = figure;
 which_plot = 'random';
   ...
   % this section does the plots during iterations   
    if plotflg==1      
      if (rem(i,df) == 0 ) | (i==me) | (i==1) 
         ...
         fprintf('best element is %d\n',idx1)
         cnt = cnt+1; % count how many times we display (useful for movies)
          
         %eval(plotfcn); % defined at top of script
         
         %added by elco for the purposes of debugging the 1 cell
         %optimisaiton
         
         %pbestval,pbest are the best scores/positions for each particle
         %out,pos are the current scores/positions for each particle
         
         switch which_plot
             case 'best'
         %plot the best particles
         [best_scores,best_index] = sort(pbestval);
         scores_to_plot = pbestval(best_index(end-show_numbers+1:end));
         radii_to_plot = pbest(best_index(end-show_numbers+1:end),:);
         
             case 'random'
         %plot random particles
          %plot the best particles
         best_index = randperm(ps);
         scores_to_plot = pbestval(best_index(end-show_numbers+1:end));
         radii_to_plot = pbest(best_index(end-show_numbers+1:end),:);
         
             case 'worst'
                 %plot the worst particles
                 [best_scores,best_index] = sort(pbestval);
         scores_to_plot = pbestval(best_index(1:show_numbers));
         radii_to_plot = pbest(best_index(1:show_numbers),:);
         ...
         for showI = 1:show_numbers
             
             [~,Xshow,Yshow,~,~] = snake_cost_fun_radial_full(show_image,center,angles,radii_to_plot(showI,:)',0,0,0,0);
             plot(Xshow,Yshow,colours(showI));
             drawnow;
             fprintf('the score for %s is %g \n',colours(showI),scores_to_plot(showI))
