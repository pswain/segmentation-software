...
% determines whether all fields below that checkbox and up to the next
% separator are initially disabled (true) or not (false).
%
% Example:
% 
...
% 
% there are two other ways to provide inputs. One is by putting everything
% as a structure with fields 'name_of_property' and property values as the
% value of this property in the structure. This structure is then the first
% input and following inputs can be of the form ...'name',value,...
% example:
%  [settings, button] = settingsdlg(...
%       struct('Description', 'This dialog will set the parameters used by FMINCON()',... 
%       'title'      , 'FMINCON() options',...
%       'Tolerance_on_Constraints', struct('entry_name',{{'some sentence with spaces','a'}},'entry_value',11),...
%       'list',{{'a1' 'a2'}}))
%
% note the double cell on the list - necessary when declaring a value in a
% structure to be a cell. Another way is to give the value arbitrary name
% and then make is a structure with the fields 'entry_name','entry_value'.
% In this case the original field name is ignored and the dialog box
% behaves as though the field name was the 'entry_name' value and the field
% value was 'entry_value' (though appended to the end of the list).
% example:
% [settings, button] = settingsdlg(...
%       struct('Description', 'This dialog will set the parameters used by FMINCON()',... 
%       'title'      , 'FMINCON() options',...
%       'forgotten_field_1', struct('entry_name',{{'Tolerance on Constraints','TolX'}},'entry_value',1e-6),...
%       'lst',{{'a1' 'a2'}}))
%
...
% https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=6G3S5UYM7HJ3N
    ...
    %cheeky addition made by Elco to allow entry of values as structures
    ii = 1;
    while(ii<=numel(parameters))
        if isstruct(values{ii})
            
            parameters{end+1} = values{ii}.entry_name;
            values{end+1} = values{ii}.entry_value;
            parameters(ii) = [];
            values(ii) = [];
        else
            ii = ii+1;
