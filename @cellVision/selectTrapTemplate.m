...
if nargin<3
    ...
    if cTimelapse.trapsPresent
        ...
        h=figure;set(gcf,'name','Select the center of a representative trap with 1 cell');imshow(image,[]);
        [x y]=getpts(gca);
        x=floor(x);y=floor(y);
        ...
        h=figure;set(gcf,'name','Select the center of a representative trap with several cells');imshow(image,[]);
        [x y]=getpts(gca);
        close(h);
        x=floor(x);y=floor(y);
