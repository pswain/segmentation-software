function filt_feat=createImFilterSetCellTrapEdges(cCellSVM,image)
...
sigma=.2;
h(:,:,1) = fspecial('gaussian', 15, sigma);
h(:,:,2) = fspecial('gaussian', 15, 10*sigma);
...
for i=1:size(filt_im,3)
    [accum, circen, cirrad] =  CircularHough_Grd(filt_im(:,:,i), [cCellSVM.radiusSmall floor((cCellSVM.radiusLarge-cCellSVM.radiusSmall)/2)+cCellSVM.radiusSmall],max(max(filt_im(:,:,i)))*.05,4,1);
    temp_im=accum;
    temp_im(cCellSVM.cTrap.trapOutline>0)=0;
    temp_index=temp_index+1;
    filt_feat(:,temp_index)=temp_im(:);

    temp_im = imfilter((accum),fspecial('disk',3),'replicate');
    temp_im(cCellSVM.cTrap.trapOutline>0)=0;
    temp_index=temp_index+1;
    filt_feat(:,temp_index)=temp_im(:);
    
    [accum, circen, cirrad] =  CircularHough_Grd(filt_im(:,:,i), [ceil((cCellSVM.radiusLarge-cCellSVM.radiusSmall)/2)+cCellSVM.radiusSmall cCellSVM.radiusLarge],max(max(filt_im(:,:,i)))*.05,4,1);
    temp_im=accum;
    temp_im(cCellSVM.cTrap.trapOutline>0)=0;
    temp_index=temp_index+1;
    filt_feat(:,temp_index)=temp_im(:);
...
for i=1:size(filt_im,3)
    ...
    for j=1:1
        temp_im=es_im(:,:,j);
        temp_im=im2bw(temp_im,graythresh(temp_im)/2);
