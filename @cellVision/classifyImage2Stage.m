...
[predict_label, accuracy, dec_values] = predict(ones(size(image,1)*size(image,2),1), sparse(filtered_image), cCellSVM.SVMModelLinear); % test the training data]\
...
bw=imdilate(bw,se1);
loc=bw>0;
[predict_label_kernel, accuracy, dec_values_kernel] = svmpredict(ones(sum(loc),1), (filtered_image(loc,:)), cCellSVM.SVMModel); % test the training data]\
