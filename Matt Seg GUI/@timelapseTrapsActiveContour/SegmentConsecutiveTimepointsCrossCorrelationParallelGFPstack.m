...
%performed on each slice and some so far un decided operation used to
%decide which edge score to use (probably min of the stack).
%Best if cCellVision has some number that is roughly mean cell brightness
%in the cCellSVM.se.mean_brightness field.
%
...
for TP = Timepoints
    ...
    for chi = 1:length(CrossCorrelationChannel)
        if chi==1
            ...
            WholeImage = cat(3,WholeImage,sign(CrossCorrelationChannel(chi)) * ttacObject.ReturnImage(TP,abs(CrossCorrelationChannel(chi))));   
        end
    end
    WholeImage = mean(WholeImage,3);
    WholeImage = WholeImage/mean_brightness;
    
    
    %Since it is a GFP segmentation ignore traps
    WholeTrapImage = zeros(size(WholeImage));

    ACImageChannel = ttacObject.Parameters.ImageTransformation.channel;
    
    % do min z project of z stack. My hope is that cell pixels will be
    % reasonably bright in all slices, whereas non cell pixels will have
    % vey low values in some slices.
    for chi = 1:length(ACImageChannel)
        if chi==1
            ACImageStack =sign(ACImageChannel(chi)) * ttacObject.ReturnImage(TP,abs(ACImageChannel(chi)));
        else
            ACImageStack = cat(3,ACImageStack,sign(ACImageChannel(chi)) * ttacObject.ReturnImage(TP,abs(ACImageChannel(chi))));   
        end
    end
    ACImageStack = ACImageStack/mean_brightness;
    ACImage = min(ACImageStack,[],3);
        ...
        %fprintf('lin 348 SegmentConsecutive....: make parallel again\n')
            ...
            while CellSearch
                ...
                if ProceedWithCell
                    ...
                    ParCurrentTrapInfo.cell(NCI).foundCellCenter = double([xnewcell ynewcell]);
                    ...
                    % Try and get the most in focus image of the cell
                    slice_sum = [];
                    for slicei = 1:length(ACImageChannel)
                        ACTrapImagei = ACBackGroundFunctions.get_cell_image(ACImageStack(:,:,slicei),...
                                                                TrapImageSize,...
                                                                [[SliceableTrapLocations(TI).xcenter]' [SliceableTrapLocations(TI).ycenter]']);
                        y_range = ynewcell + (-1:1);
                        y_range(y_range<1) = [];
                        y_range(y_range>TrapImageSize(1)) = [];
                        
                        x_range = xnewcell + (-1:1);
                        x_range(x_range<1) = [];
                        x_range(x_range>TrapImageSize(2)) = [];
                        cell_centre_section = ACTrapImagei(y_range, x_range);
                        slice_sum = [slice_sum sum(cell_centre_section(:))];
                    end
                    
                    [~,best_slicei] = max(slice_sum);
                    ACTrapImage = ACBackGroundFunctions.get_cell_image(ACImageStack(:,:,best_slicei),...
                                                                TrapImageSize,...
                                                                [[SliceableTrapLocations(TI).xcenter]' [SliceableTrapLocations(TI).ycenter]']);
                    ...
                    if ACparameters.visualise >= 5;
                        figure(15);
                        imshow(OverlapGreyRed(TransformedCellImage,ExcludeLogical,[],[],true),[]);
                        figure(16);
                        imshow(CellImage,[]);
                        figure(17);
                        imshow(ACTrapImage,[]);
                        imtool(TransformedCellImage,[]);
                        uiwait()
                    end
                    ...
                    ParCurrentTrapInfo.cell(NCI).foundCellRadii = RadiiResult;
