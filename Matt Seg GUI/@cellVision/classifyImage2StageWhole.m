...
tempCurreTpOutline=cCellSVM.cTrap.currentTpOutline;
...
[predict_labelLin, ~, dec_valuesLin] = predict(labels(~trapOutlineTrapsOnly(:)), sparse(filtered_image(~trapOutlineTrapsOnly(:),:)), cCellSVM.SVMModelLinear); % test the training data]\
...
b=dec_valuesTrapsOnly;
[B,IX]=sort(b(:),'ascend');

l=sum(~trapOutlineTrapsOnly(:))*.055;
loc=IX(1:(round(l)));
...
cCellSVM.cTrap.currentTpOutline=tempCurreTpOutline;
