function [bVar, bw] = snakeIterate(cCellVision,bVar,image, xs, ys, alpha, beta, gamma, kappa, wl, we, wt, iterations)
...
for i=1:N;
    ...
    %below is to prevent crashes when the circle goes outside the image
    loc=find(xs>nn | xs<1 | ys>mm | ys<1);
    if ~isempty(loc);
        break;
    end
...
% plot([xs; xs(1)], [ys; ys(1)], 'r-');
% 
bw=poly2mask(xs,ys,size(image,1),size(image,2));
