    ...
    methods
        function CellResGUI=cellResultsViewingGUI(cExperiment)
            ...
            if length(cExperiment.cellInf) ~= length(cExperiment.cTimelapse.channelNames)
                if isfield(cExperiment.cellInf,'channelName')
                    CellResGUI.ChannelDataCode = {cExperiment.cellInf.channelName}';
                elseif ~isfield(cExperiment.cellInf,'extractionParameters')...
