function filt_feat=createImFilterSetNoTrapSlim100x(cCellSVM,image)
...
Fgaussian =fspecial('gaussian',16,5);
...
for i=1:size(im,3)
    ...
    temp_im=imfilter(temp_im,Fgaussian,'replicate');
    temp_index=temp_index+1;
    filt_feat(:,temp_index)=temp_im(:);
    %filt_im(:,:,size(im,3)+temp_index)=temp_im;
end
...
for i = 1:size(im,3)
    ...
    temp_index = temp_index+1;
    temp_im=imfilter(IMoutPOS,Fgaussian,'replicate');
    filt_feat(:,temp_index) = temp_im(:);
    ...
    temp_index = temp_index+1;
    temp_im=imfilter(IMoutNEG,Fgaussian,'replicate');
    filt_feat(:,temp_index) = temp_im(:);
