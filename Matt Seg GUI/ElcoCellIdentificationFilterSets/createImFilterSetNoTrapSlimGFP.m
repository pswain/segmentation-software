function filt_feat=createImFilterSetNoTrapSlimGFP(cCellSVM,image)
% filt_feat=createImFilterSetNoTrapSlimGFP(cCellSVM,image)
% designed specifically for stacks of GFP images (bright ones)
% expects cCellSVM.se to have a field called 'mean_brightness'. Should be
% set to be roughly the mean bightness of cell pixels in the itmelapse
% (maybe set it from histograms or whatever, hopefully not super
% important).
...
if ~isfield(cCellSVM.se,'mean_brightness') || isempty(cCellSVM.se.mean_brightness)
    mean_brightness = 1;
else
    mean_brightness = cCellSVM.se.mean_brightness;
end
