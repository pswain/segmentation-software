...
%             extractedData(channel).pixel_sum(:,timepoint)=ed_pixel_sum;
            extractedData(channel).membraneMedian(:,timepoint)=sparse(ed_membraneMedian);
            extractedData(channel).membraneMax5(:,timepoint)=sparse(ed_membraneMax5);
            extractedData(channel).smallmax5(:,timepoint)=sparse(ed_smallmax5);
%             extractedData(channel).smallmedian(:,timepoint)=ed_smallmedian;
%             extractedData(channel).smallmean(:,timepoint)=ed_smallmean;
            extractedData(channel).imBackground(:,timepoint)=sparse(ed_imBackground);
            if channel==1
                extractedData(channel).area(:,timepoint)=sparse(ed_area);
                extractedData(channel).radius(:,timepoint)=sparse(ed_radius);
                extractedData(channel).eccentricity(:,timepoint)=sparse(ed_eccentricity);
%                 extractedData(channel).radiusFL(:,timepoint)=ed_radiusFL;
                extractedData(channel).segmentedRadius(:,timepoint)=sparse(ed_segmentedRadius);
%                 extractedData(channel).nucArea(:,timepoint)=ed_nucArea;
%                 extractedData(channel).distToNuc(:,timepoint)=ed_distToNuc;
                extractedData(channel).radiusAC(:,timepoint)=sparse(ed_radiusAC);
                extractedData(channel).xloc(:,timepoint)=sparse(ed_xloc);
                extractedData(channel).yloc(:,timepoint)=sparse(ed_yloc);
            end
%             extractedData(channel).pixel_variance_estimate(:,timepoint)=sparse(ed_pixel_variance_estimate;
