function identifyCellBoundaries(cTimelapse,cCellVision,timepoint,traps,channel, method,bw)
...
switch method
    case 'hough'
        hough_track(cTimelapse,cCellVision,traps,channel,timepoint,bw)
    case 'active_contour'
        linear_segmentation(cTimelapse,cCellVision,traps,channel)
end
end

function hough_track(cTimelapse,cCellVision,traps,channel,timepoint,bw_mask)
s1=strel('disk',1);
% for i=1:length(cTimelapse.cTrapsLabelled(traps(1)).timepoint)    
    ...
    for j=1:size(image,3)
        ...
        if isempty(bw_mask)
            bw_mask=cTimelapse.cTimepoint(timepoint).trapInfo(traps(j)).segCenters;
            bw_mask=imdilate(bw_mask,s1);
        end
        [accum circen cirrad] =CircularHough_Grd_matt(temp_im,[cCellVision.radiusSmall cCellVision.radiusLarge],bw_mask);
        bw_mask=[];
        
        cTimelapse.cTimepoint(timepoint).trapInfo(traps(j)).cellCenters=circen;
        cTimelapse.cTimepoint(timepoint).trapInfo(traps(j)).cellRadius=cirrad;
        ...
        for k=1:length(cirrad)
            ...
            cTimelapse.cTimepoint(timepoint).trapInfo(traps(j)).segmented(:,:,k)=temp_im;
        end
    end
end
...
function [accum, varargout] = CircularHough_Grd(img, radrange, mattmask, varargin)
