function identifyCellObjects(cTimelapse,cCellVision,traps, channel, method)
...
switch method
    case 'hough'
        hough_track(cTimelapse,cCellVision,traps,channel)
    case 'linear'
...
function hough_track(cTimelapse,cCellVision,traps,channel)
...
for i=1:length(cTimelapse.cTrapsLabelled(traps(1)).timepoint)
    ...
    for j=1:size(image,3)
        ...
        bw_mask=cTimelapse.cTrapsLabelled(traps(j)).segmented(:,:,i);

        [accum circen cirrad] =CircularHough_Grd_matt(temp_im,[cCellVision.radiusSmall cCellVision.radiusLarge],bw_mask);

        cTimelapse.cTrapsLabelled(traps(j)).cellCenters{i}=circen;
        cTimelapse.cTrapsLabelled(traps(j)).cellRadius{i}=cirrad;
    end
end
end
